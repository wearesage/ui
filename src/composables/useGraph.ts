...
// Extend SimulationNodeDatum to create our D3Node type
...
export function useGraph(canvas: Ref<HTMLCanvasElement | null>) {
  ...
  const {
    ...
    circle,
    mouse,
  } = useCanvas2d(canvas, viewport.artboard);

  // Define a type for our simulation
  ...
  // Helper function to get a readable label for a node
  function getNodeLabel(node: NodeData): string {
    if (!node.properties) return node.id;
    
    // Use the most appropriate property based on node type
    ...
    // Fallback to first label type or id
  ...
  function buildSimulation(nodes: NodeData[], edges: EdgeData[], viewport: Viewport): void {
    ...
    simulation.value = forceSimulation(d3Nodes)
      ...
      .force(
        "center",
        forceCenter(viewport.artboard.width / 2, viewport.artboard.height / 2)
      )
      .on("tick", drawGraph);
  }
