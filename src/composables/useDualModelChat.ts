...
export function useDualModelChat(options: DualModelOptions = {}) {
  ...
  const messages = ref<Message[]>(options.messages || [])
...
If no tool is appropriate for the query, don't use any tools.`
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/api/chat`, {
        ...
        body: JSON.stringify({
          ...
          num_ctx: options.num_ctx || Math.pow(2, 14),
          temperature: 1.5
        }),
  ...
  const streamMessage = async ({ content, images }: { content: string, images: any[] }, onUpdate?: (text: string) => void) => {
    try {
      ...
      for (let i = lastMessages.length - 1; i >= 0; i--) {
        ...
        if (lastMessages[i].role === 'user' && lastMessages[i].content === content) {
      ...
      if (needsToolCalls) {
        ...
        if (toolCalls.length > 0) {
          ...
          return await streamConversationModelResponse(content, images, results, onUpdate)
        } else {
